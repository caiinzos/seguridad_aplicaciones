...
public class OrderController {
    ...
    public String doGetOrderPage(@SessionAttribute(Constants.USER_SESSION) User user,
        ...
        try {
            ...
            if (!order.getUser().getUserId().equals(user.getUserId())) {
                logger.warn("SECURITY: User {} attempted to access order {} without ownership",
                        user.getUserId(), id);
                return errorHandlingUtils.handleAccessDeniedException(
                        "Acceso denegado: No tienes permisos para ver este pedido", model, locale);
            }
    ...
    public String doGetPaymentPage(@SessionAttribute(Constants.USER_SESSION) User user,
        ...
        try {
            ...
            if (!order.getUser().getUserId().equals(user.getUserId())) {
                logger.warn("SECURITY: User {} attempted to access payment for order {} without ownership",
                        user.getUserId(), id);
                return errorHandlingUtils.handleAccessDeniedException(
                        "Acceso denegado: No tienes permisos para pagar este pedido", model, locale);
            }
    ...
    public String doPayOrder(@Valid @ModelAttribute(Constants.ORDER_FORM) PaymentForm paymentForm,
        ...
        try {
            Order order = orderService.findById(id);
            if (!order.getUser().getUserId().equals(user.getUserId())) {
                logger.warn("SECURITY: User {} attempted to pay order {} without ownership",
                        user.getUserId(), id);
                return errorHandlingUtils.handleAccessDeniedException(
                        "Acceso denegado: No tienes permisos para pagar este pedido", model, locale);
            }
    ...
    public String doCancelOrder(@PathVariable() Long id,
        ...
        try {
            Order order = orderService.findById(id);
            if (!order.getUser().getUserId().equals(user.getUserId())) {
                logger.warn("SECURITY: User {} attempted to cancel order {} without ownership",
                        user.getUserId(), id);
                return errorHandlingUtils.handleAccessDeniedException(
                        "Acceso denegado: No tienes permisos para cancelar este pedido", model, locale);
            }
